...
@bot.message_handler(
    func=lambda mess: mess.text == const.emoji['anticlockwise'] + ' Замены' or
                      mess.text == '« Назaд',
    content_types=['text'])
...
def calendar_replace_handler(message):
    ...
    answer = ('Выбери день:\n' + const.emoji[
        'check_mark'] + ' – Есть замены для ' + for_any + ' <b>{0}</b>.\n'.format(
        group)
    ...
    if len(y) == 6:
        ...
        week_day_calendar.row(
            *[tb.types.InlineKeyboardButton(text=name, callback_data=name) for
              name in
              [const.emoji['negative_squared_cross_mark'] + ' Вся нeделя']])
    elif len(cross) == 6:
        week_day_calendar.row(
            *[tb.types.InlineKeyboardButton(text=name, callback_data=name) for
...
def sending_handler(message):
    answer = (
            'Выбери, на рассылку чего ты хочешь <b>подписаться</b> или <b>отписаться</b>:\n'
            + const.emoji['check_mark'] + ' – Рассылка включена.\n'
            + const.emoji[
                'negative_squared_cross_mark'] + ' – Рассылка отключена.')
    sql_con = connect(const.path + 'Bot.db')
    cursor = sql_con.cursor()
...
@bot.message_handler(
    func=lambda mess: mess.text == const.emoji['magnifying_glass'] or
                      mess.text == '« Нaзад',
    content_types=['text'])
...
@bot.message_handler(func=lambda mess: mess.reply_to_message is not None and
                                       mess.reply_to_message.from_user.username ==
                                       bot_username and str(
    mess.chat.id) in const.admin_ids and
...
def banned_user_inline_handler(call_back):
    answer = const.emoji['no_entry'] + (
        ' Вы заблокированы по причине неоднократного спама! '
...
@bot.callback_query_handler(func=lambda call_back: reg_func.get_step(
    call_back.message.chat.id) == 'select_teacher' and
                                                   (call_back.data.split('|')[
                                                        0] in const.teacher_name or
                                                    call_back.data.split('|')[
                                                        0] in const.sht_teachers or
                                                    call_back.data.split('|')[
                                                        0] in const.cap_teachers))
        ...
        if const.emoji['negative_squared_cross_mark'] in call_back.data:
            ...
            try:
                for date in answerr:
                    ...
                    all_dates.append(
                        datetime(int(str_date[2]), int(str_date[1]),
                                 int(str_date[0])))
                all_dates.sort()
                ...
                answer = (const.emoji[
                              'anticlockwise'] + ' Нет замен с <b>' + first_date +
                          '</b> по <b>' + second_date + '</b>.')
            except:
                bot.edit_message_text(
                    text='<b>Упс, что-то пошло не так\U00002026</b>\n' + str(
                        exc_info()[1]),
                    chat_id=call_back.message.chat.id,
                    message_id=call_back.message.message_id,
                    parse_mode='HTML',
            ...
            back_from_week_replace_handler.row(
                *[tb.types.InlineKeyboardButton(text=name, callback_data=name)
                  for
                  name in ['« Hазад']])
            for i in range(1, 7):
                if i == 1:
                    ...
                    bot.send_message(call_back.message.chat.id, answerr[
                        i - 1], parse_mode='HTML',
                                     reply_markup=back_from_week_replace_handler)
                else:
                    bot.send_message(call_back.message.chat.id, answerr[
...
def sending_on_not_repl_handler(call_back):
    ...
    back_from_podpis_handler.row(
        *[tb.types.InlineKeyboardButton(text=name, callback_data=name) for
          name in ['« Назад']])
    answer = (
        '{0} Рассылка об отсутствии замен <b>активирована</b>\nЖди рассылку после опубликования замен.'
        ''.format(const.emoji['mailbox_on']))
    bot.edit_message_text(text=answer,
                          chat_id=call_back.message.chat.id,
                          message_id=call_back.message.message_id,
...
def sending_off_not_repl_handler(call_back):
    ...
    back_from_podpis_handler.row(
        *[tb.types.InlineKeyboardButton(text=name, callback_data=name) for
          name in ['« Назад']])
    answer = '{0} Рассылка об отсутствии замен <b>отключена</b>'.format(
        const.emoji[
            'mailbox_off'])
    bot.edit_message_text(text=answer,
                          chat_id=call_back.message.chat.id,
        ...
        if message.text.split()[0].isdigit():
            ...
            if chat_id.isdigit():
                ...
                try:
                    ...
                    bot.send_message(message.chat.id, const.emoji[
                        'cross_mark'] + '\n' + str(err))
        elif message.text[:3].lower() == 'ban' and message.text[3:].replace(' ',
                                                                            '') != conf.my_id:
            try:
                if func.is_user_not_banned(int(message.text[3:])):
                    ...
                    remove_keyboard = tb.types.ReplyKeyboardRemove()
                    bot.send_message(int(message.text[3:]), const.emoji[
                        'cross_mark'] + ' Заблокирован',
                                     reply_markup=remove_keyboard)
            ...
            try:
                if func.is_user_banned(int(message.text[5:])):
                    ...
                    bot.send_message(int(message.text[5:]), const.emoji[
                        'check_mark'] + ' Разблокирован',
                                     reply_markup=main_keyboard_btn)
                    ...
                    if message.text.split()[1].lower() == 'len':
                        ...
                        for si in send_ids:
                            if si in not_banned_users:
                                try:
                                    if str(si[0]).isdigit():
                                        bot.send_message(si[0], message.text[
                                                                len(
                                                                    message.text.split()[
                                                                        0]):],
                                                         True,
